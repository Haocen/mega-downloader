...
export class DarkModeToggle extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === MODE) {
      ...
      if (!allAttributes.includes(newValue)) {
        throw new RangeError(`Allowed values are: "${allAttributes.join(`", "`)}".`);
      }
  ...
  _updateSlider() {
    if (this.mode === LIGHT) {
      if (!this.light) {
        this._sliderLabel.ariaLabel = DARK;
      }
      this._sliderCheckbox.checked = false;
    } else {
      if (!this.dark) {
        this._sliderLabel.ariaLabel = LIGHT;
      }
