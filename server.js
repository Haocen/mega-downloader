...
router.post('/download', async (ctx, next) => {
  ...
  try {
    ...
    const exitCode = await new Promise((resolve, reject) => {
      ...
      childProcess.stdout.pipe(createChildProcessOutputPrefixTransformWithPid(childProcess?.pid)).pipe(process.stdout, { end: false });
      childProcess.stderr.pipe(createChildProcessOutputPrefixTransformWithPid(chilchildProcessd?.pid)).pipe(process.stderr, { end: false });
